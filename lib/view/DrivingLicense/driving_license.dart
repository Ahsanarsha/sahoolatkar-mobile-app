...
import 'package:saholatkar/controller/ProfileMenu/AccountLevel/driving_license.dart';
...
class DrivingLicenseScreen extends StatefulWidget {
  ...
  const DrivingLicenseScreen({Key? key, required this.drivingLicense})
      : super(key: key);
...
class _DrivingLicenseScreenState extends State<DrivingLicenseScreen> {
  ...
  void initState() {
     if(controller.model.records!=null && controller.model.records!.isNotEmpty ) {
      url = '${ApiConstants.BASE_URL}DrivingLisence/${controller.model.records![0].drivingLicense}';
    }
    print(imageFile);
    super.initState();
  }
  ...
  /// Get from gallery
  _getFromGallery({required DrivingLicenseController licenseController}) async {
  ...
  /// Get from Camera
  _getFromCamera({required DrivingLicenseController licenseController}) async {
    XFile? pickedFile = await ImagePicker().pickImage(
  ...
  Widget build(BuildContext context) {
    return GetBuilder<DrivingLicenseController>(
      init: DrivingLicenseController(),
      builder: (licenseController) => Scaffold(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      ...
                      child: CustomButtonWithoutBold(
                        ...
                        onPressed: () {
                          _getFromGallery(licenseController: licenseController);
                        },
                      ),
                    ),
                    ...
                    Expanded(
                      ...
                      child: GestureDetector(
                        onTap: () {
                          _getFromCamera(licenseController: licenseController);
                        },
                        child: Text(
                          "Take Photo",
                ...
                CustomButtonWithoutBold(
                  textColor: Colors.white,
                  textValue: widget.drivingLicense.id != 0 ? "Next" : "Add",
                  buttonColor: HexColor("1BCB5D"),
                  onPressed: () async {
                    if (widget.drivingLicense.id != 0) {
                    } else {
                      if (imageFile!.path.isNotEmpty) {
                        ...
                        await licenseController.uploadDrivingLicense(
